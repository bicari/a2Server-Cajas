...
async def search_database_files()  -> bool :
        try:
            LIST_TABLES = ['SOperacionInv', 'SdetalleVenta'] #Lista de tablas a enviar al servidor desde la caja
            file_compress = zipfile.ZipFile('data.zip', 'w')
            result = next(os.walk('C:\\a2CA2020\\Empre001\Data'))[2] #Lista de archivos del directorio local de la caja
            for file in result:
